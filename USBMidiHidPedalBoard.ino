...
class myMidi : public USBMIDI {
 virtual void handleNoteOff(unsigned int channel, unsigned int note, unsigned int velocity) {
   digitalWrite(LED_BUILTIN, HIGH ); // Turn off LED briefly to indicate USB input
 ...
 int getChannelNoteIndex( unsigned int channel, unsigned int note ) {
     for ( int i = 0 ; i < MAX_SCANNED_GPIO  + SHIFT_REGISTER_BITS ; i++ ) {
       switch ( ScanParams[i].command ) {
        case MIDI_NOTE_CH1 :
        case MIDI_NOTE_CH2 :
        case MIDI_NOTE_CH3 :
        case MIDI_NOTE_CH4 :
          if ( ( ScanParams[i].command -  MIDI_NOTE_CH1 ) == channel )
        ...
        case MIDI_NOTE_TOGGLE_CH4 :
          if ( ( ScanParams[i].command - MIDI_NOTE_TOGGLE_CH1 ) == channel )
            if ( ScanParams[i].value == note )
              return i;
        break;
       }
    ...
    switch ( ScanParams[scanId].command ) {
      ...
      case MIDI_NOTE_CH4 : 
        {    
          ...
          delay(5);
          digitalWrite(LED_BUILTIN, LOW ); // Indicate USB signalling end PCB LED ON
        }
        break;
