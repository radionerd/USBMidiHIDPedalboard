...
//  { PB7,  IP_SR_DATA   }, // Blue Pill
  { PB6,  IO_RESERVED }, // I2C SCL1 LCD Displays
  { PB7,  IO_RESERVED }, // I2C SDA1
...
//USBMultiSerial<1> MultiSerial; // One multiserial object
...
class myMidi : public USBMIDI {
  ...
  virtual void handleSysExEnd(void) {
    ...
    const int SysExLCDMessageIdentifier=1;
    const int SysExLCDNumberMSB=0;
    const int SysExLCDColourIndex=5; // 1 = White Unused
    const int SysExLCDTextStartIndex=6;

    sysexBuf[sysexIndex-1] = 0;
    sysexIndex=0; // Null terminate in leiu of end flag
    if ( sysexBuf[ 0 ] == SysExStartFlag && sysexBuf[ 1 ] == SysExManufacturerID && sysexBuf[ 2 ] == SysExLCDMessageIdentifier && sysexBuf[ 4 ] == SysExLCDNumberMSB ) {
      uint8_t LCDNumberLSB = sysexBuf[3];
      LCD_N_C32 lcd;
      lcd.write( LCDNumberLSB, sysexBuf + SysExLCDTextStartIndex );
    } else {
      CompositeSerial.write("SysEx: ");
      for ( int i = 0 ; i < 7 ; i++ ) {
        char buff[20];
...
void setup() {
  ...
  USBComposite.setProductString((const char *)"Organ-Midi,HID,Serial");
...
void loop() {
  ...
  profile.log ("IOScan");
  midi.poll();
  profile.log ("midi.poll");
  led.service();
  profile.log ("led.service");
  while (CompositeSerial.available()) {
    char buff[20];
    char c = CompositeSerial.read();
    sprintf(buff, "Input=%02x %c\n", c, c);
    switch ( tolower(c) ) {
      case 'r' :
        CompositeSerial.write(buff); // serial port is provisioned to assist reset when installing code using a boot loader
        CompositeSerial.write("\n");
        CompositeSerial.write(__FILE__);
        CompositeSerial.write("\n");
        CompositeSerial.write(__DATE__);
        CompositeSerial.write("\n");
        CompositeSerial.write(__TIME__);
        CompositeSerial.write("\n");
      break;
      case 'p' :
        char buff[80];
      ...
      default:   
        CompositeSerial.write("Help:\n"); // serial port is provisioned to assist reset when installing code using a boot loader
        CompositeSerial.write("P - Profile\n"); // serial port is provisioned to assist reset when installing code using a boot loader
        CompositeSerial.write("R - Release Info\n"); // serial port is provisioned to assist reset when installing code using a boot loader
    }
  }
}
